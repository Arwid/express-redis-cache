  ...
  function route () {
    ...
    domain.run(function () {
      ...
      middleware = function expressRedisCache_Middleware (req, res, next) {
        ...
        var cacheControlHeader = self.cacheControlHeader || {xxx: false};
        if ( typeof options[0] === 'object' ) {
          ...
          if ( typeof options[0].cacheControlHeader === 'string' || typeof options[0].expire === 'object' || typeof options[0].expire === 'function') {
            cacheControlHeader = options[0].cacheControlHeader;
          }
        }

        if ( typeof options[0] === 'number' ) {
          expire = options[0];
          if ( typeof options[1] === 'string' || typeof options[1] === 'object') {
            cacheControlHeader = options[1]
          }
        }
        else if ( typeof options[1] === 'number' || typeof options[1] === 'object') {
          expire = options[1];
          if ( typeof options[2] === 'string' || typeof options[2] === 'object') {
            cacheControlHeader = options[1]
          }
        }
        ...
        var cacheControlPolicy = require('./cacheControl')(cacheControlHeader);

        var addCacheControlHeader = function(cacheControlHeader) {
        ...
        self.get(name, domain.bind(function (error, cache) {
          ...
          if ( cache.length ) {
            addCacheControlHeader(cache[0].cacheControlHeader)
          ...
          /** otherwise, cache request **/
            ...
            res.send = function (body) {
              ...
              var expire = expirationPolicy(req, res);
              var cacheControlHeader = cacheControlPolicy(req, res);
              addCacheControlHeader(cacheControlHeader);
